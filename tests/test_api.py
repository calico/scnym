...
scnym_interpret = scnym.api.scnym_interpret
...
def test_scnym(n_epochs: int = 1, n_cells: int = 2048):
    ...
    # filter multiplets and unannotated cells
    adata = adata[
        (adata.obs["multiplets"] != "doublet") & (adata.obs["cell"] != "nan")
    ].copy()
    # label target data as stimulated condition
...
def test_scnym_interpret():
    ...
    # Test interpretation
    ##########################################
    print("Performing interpretation...")
    interpretation = scnym_interpret(
        adata=adata,
        groupby="cell",
        source="CD4 T cells",
        target="B cells",
    ...
    print("Saliency")
    print(interpretation["saliency"])
    print()
    print("Gradients")
    print(interpretation["gradients"].head(5))
    print()
    print("Done.")
    return
...
def main():
    ...
    logging.debug("Debugging logger active.")

    print("<-- test_scnym")
    #     test_scnym(n_epochs=50, n_cells=None)
    ...
    #     print('<-- test_scnym_interpret')
    #     test_scnym_interpret()
    return
